...
# Check for pactl early, as it's essential
if ! command -v pactl &> /dev/null; then
    echo "ERROR: 'pactl' command not found. This script requires 'pactl' to function." >&2
    # Try to send a notification about the missing dependency if notify-send is available
...
get_current_volume_percentage() {
    ...
    if [[ -z "$volume_output" ]] || ! [[ "$volume_output" =~ ^[0-9]+$ ]]; then
        ...
        # Try to notify about parsing error
        if command -v "$(dirname "$0")/notify.sh" &> /dev/null; then
             "$(dirname "$0")/notify.sh" -u normal -a "Volume Script Error" -t "Volume Error" -m "Could not parse volume from pactl."
        elif command -v notify-send &> /dev/null; then # Fallback if notify.sh somehow fails or isn't found yet
            notify-send -u normal -a "Volume Script Error" "Error: Could not parse volume from pactl."
        fi
...
process_mic_mute_status() {
    local app_name="Microphone" # Changed from notification_title
    local icon_path
    local display_text
    local percentage_value # For progress bar in notify.sh
...
process_volume_status() {
    local app_name="Volume" # Changed from notification_title
    local current_volume
    current_volume=$(get_current_volume_percentage)
    local icon_path
    local display_text # This will be the message for notify.sh
    local percentage_value # For progress bar
...
main() {
    if [[ "${1-}" == "MUTE" ]]; then # Check if $1 is set before comparing
        process_mic_mute_status
    else
        process_volume_status
    fi
