...
get_v(){ pactl get-sink-volume @DEFAULT_SINK@|grep -Po '[0-9]+(?=%)'|head -n 1;}
is_m(){ pactl get-sink-mute @DEFAULT_SINK@|grep -q yes;}
is_mic_m(){ pactl get-source-mute @DEFAULT_SOURCE@|grep -q yes;}
